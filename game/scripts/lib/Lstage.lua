...
function M.DestroyCurrentStage()
    if M.current_stage and type(M.current_stage.del) == "function" then
        M.current_stage:del()
    end
    -- Some exported cards delete their pool objects during del().  LuaSTG
    -- versions differ in whether a second ResetPool call is tolerated; keep
    -- the stage transition alive when the native pool is already empty.
    local ok, err = pcall(lstg.ResetPool)
    if not ok then
        print("TNR: ResetPool during stage transition was ignored: " .. tostring(err))
    end
