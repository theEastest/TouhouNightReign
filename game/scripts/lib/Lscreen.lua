...
function SetViewMode(mode)
    ...
    if mode == '3d' then
        ...
        -- Several original reference backgrounds use a zero near clipping
        -- plane (for example karl_icepool).  LuaSTG Sub now rejects that
        -- value in SetPerspective, so normalize only the invalid boundary
        -- while preserving the source camera's far plane and projection.
        local view_z = lstg.view3d.z or {}
        local z_near = tonumber(view_z[1]) or 0.001
        local z_far = tonumber(view_z[2]) or 5
        z_near = math.max(0.001, z_near)
        if z_far <= z_near then
            z_far = z_near + 0.001
        end
        SetPerspective(
