...
-- A native/network room can provide one authoritative input frame without
-- touching the physical keyboard state.  The override is consumed by
-- update() after the normal device mapping, so legacy player systems see the
-- same action values as a local replay.
local external_input_state
...
function M.setExternalInput(state)
    external_input_state = state
end

function M.clearExternalInput()
    external_input_state = nil
end
...
function M.update()
    ...
    if external_input_state then
        local state = external_input_state
        local move_x = tonumber(state.move_x) or 0
        local move_y = tonumber(state.move_y) or 0
        local directional_defaults = {
            up = move_y > 0,
            down = move_y < 0,
            left = move_x < 0,
            right = move_x > 0,
        }
        for _, name in ipairs(boolean_action_list) do
            if state[name] ~= nil then
                boolean_action_state[name] = not not state[name]
            elseif directional_defaults[name] ~= nil then
                boolean_action_state[name] = directional_defaults[name]
            end
        end
        local x = move_x
        local y = move_y
        ...
        local move = polar_vector2_action_state.move or {}
        move.r = round(clamp(r, 0.0, 1.0) * 100)
        move.a = round(math.deg(atan(y, x))) % 360
        polar_vector2_action_state.move = move
    end
end
