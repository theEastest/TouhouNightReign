...
function misc.ShakeScreen(t, s)
    local shaker = lstg.tmpvar._G_shaker
    -- Stage changes reset the native object pool, but the legacy global
    -- reference can outlive the object it points to.  Testing only for a
    -- non-nil value therefore attempts to write through an invalid userdata
    -- on the first screen shake of the next room.
    if shaker and IsValid(shaker) then
        shaker.time = t
        shaker.size = s
        shaker.timer = 0
        return
