...
function infobar:init(ui, system)
    ...
    self.t = 0
    self.mt = 15
    -- Standalone/room-bridge stages can render the HUD before the boss
    -- card system has completed its first frame. Keep the counter numeric so
    -- the original rendering code remains valid during that first frame.
    self.sc_left = (b and b.sc_left) or 0
end
function infobar:frame()
    ...
    local bscl = b.sc_left or self.sc_left or 0
...
function infobar:render()
    ...
    if _ui.drawname then
        ...
        local sc_left = (self.sc_left or b.sc_left or 0) + anisc
