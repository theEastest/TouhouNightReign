...
function system:updateBG()
    ...
    if b.bg then
        local spell_bg = IsValid(b.bg) and b.bg or nil
        if spell_bg then
            if b.__show_scbg then
                spell_bg.alpha = min(1, spell_bg.alpha + 0.025)
            else
                spell_bg.alpha = max(0, spell_bg.alpha - 0.025)
            end
        end
        local scene_bg = lstg.tmpvar and lstg.tmpvar.bg
        if scene_bg and IsValid(scene_bg) then
            if spell_bg and spell_bg.alpha == 1 then
                lstg.tmpvar.bg.hide = true
            else
                lstg.tmpvar.bg.hide = false
            end
        elseif lstg.tmpvar then
            -- `tmpvar.bg` can outlive a destroyed stage object.  Clear the
            -- stale handle so later cards cannot index it as a live object.
            lstg.tmpvar.bg = nil
        end
    end
end
