...
function item:init(x, y, t, v, angle)
    ...
    self.attract = 0
    -- Items can be created while the legacy player system is being rebuilt.
    -- Keep a target when one exists, but never assume it remains valid across
    -- a failed-room restart or player death transition.
    self.target = rawget(_G, 'player')
    -- Native co-op runs one independent item pool per process.  Keep an
    -- explicit owner on every item so a remote player proxy can never
    -- collect a drop that belongs to this local simulation.
    self._tnr_owner_player_id = tonumber(rawget(_G, 'tnr_item_owner_player_id')) or 1
end
...
function item:frame()
    ...
    elseif self.attract > 0 then
        local target_valid = player and (not IsValid or IsValid(player))
        if not target_valid then
            player = rawget(_G, 'player')
            target_valid = player and (not IsValid or IsValid(player))
        ...
        if target_valid then
            ...
            self.vx = self.attract * cos(a) + (player.dx or 0) * 0.5
            self.vy = self.attract * sin(a) + (player.dy or 0) * 0.5
        else
            -- A stale target must not make Angle/Dist receive nil. Let the
            -- item fall normally until the next player-collection pass.
            self.attract = 0
...
function item:colli(other)
    local owner_id = tonumber(self._tnr_owner_player_id)
    local local_id = tonumber(rawget(_G, 'tnr_item_owner_player_id')) or 1
    if other == player and (not owner_id or owner_id == local_id) then
