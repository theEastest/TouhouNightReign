...
float2 Distortion(float2 xy, float2 delta, float deltaLen)
{
	...
	float2 delta1 = float2(sin(1.75f * 2 * PI * delta.x + 0.05f*deltaLen + timer/5),sin(1.75f * 2 * PI * delta.y + 0.05f*deltaLen + timer/6)); //1.75f 此项越高，波纹越“破碎”
	float delta2 = arg * sin(0.005f * 2 * PI * deltaLen+ timer/10); //0.005f 此项越高，波纹越密
...
{	 	
	...
	if (deltaLen <= effectColorSize) {
		...
		float4 processedcolor=float4(max(50/255.f,effectColor.r),max(50/255.f,effectColor.g),max(50/255.f,effectColor.b),effectColor.a);
