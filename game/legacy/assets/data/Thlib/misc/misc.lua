...
LoadTexture('white_ani','THlib\\misc\\white_ani.png')
LoadImage('whiteblock','misc',48,0,32,32)
LoadImage('whitedot','misc',64,64,16,16)
LoadImageGroup('white_ani','white_ani',0,0,64,64,10,1)
...
function mask_fader:init(mode)
	...
	self.group=GROUP_GHOST
	self.open=(mode=='open')
	if self.open then BgSwitch.alpha=255 else BgSwitch.alpha=0 end
end

function mask_fader:frame()
	if self.open then BgSwitch.alpha=max(0,BgSwitch.alpha-9) else BgSwitch.alpha=min(255,BgSwitch.alpha+9) end
...
function misc.UnitRenderRing(img,x,y,r1,r2,rot,n) --当绘制目标为单位时使用
	local da=360/n
	for i=1,n do
		local a=rot-da*i
		Render4V(img,
