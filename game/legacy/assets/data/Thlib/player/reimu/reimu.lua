...
function reimu_player:init()
	...
	LoadImageGroup('reimu_drawring','boss',64,0,16,8,1,16)
	for i=1,16 do SetImageState('reimu_drawring'..i,'mul+add',Color(0x80FFFFFF)) end
	-----------------------------------------
...
--			local num=60/(self.support+1)
		if self.timer%8<4 then
			...
			for i=1,4 do
				if self.sp[i] and self.sp[i][3]>0.5 then
					New(reimu_bullet_blue,'reimu_bullet_blue',self.supportx+self.sp[i][1],self.supporty+self.sp[i][2],8,self.anglelist[num][i],self.target,900,1)
				end
			end
		end end
	end
...
--		New(bullet_killer,self.x,self.y)
		...
		for i=1,8 do
			New(reimu_sp_ef1,'reimu_sp_ef',self.x,self.y,8,rot+i*45,tar1,1200,1.5,40-10*i)
		end
		self.nextspell=300
		self.protect=360
	end
...
------------------------------------------------------
function reimu_player:special()
	if lstg.var.power>=200 then
		if self.death>90 then scoredata.reimu_cjuesi=scoredata.reimu_cjuesi+1 end
		if scoredata.reimu_cjuesi==3 then New(achievement_obj,2,17) end
		self.death=0
		self.nextsp=180
		...
		New(reimu_drawring)
		New(tasker,function()
			local g=lstg.var.graze
			task.Wait(180)
			scoredata.reimu_c_time=scoredata.reimu_c_time+180
			if scoredata.reimu_c_time>=18000 then New(achievement_obj,2,18) end
			if lstg.var.graze-g>=500 then New(achievement_obj,2,16) end
		end)
	end
end
-------------------------------------------------------
...
--	if abs(self.support-int(lstg.var.power/100))<0.0625 then self.support=int(lstg.var.power/100) end
	...
	if (self.death==0 or self.death>90) and (not self.lock) and not(self.time_stop) then
	if not self.dialog then
		if self.timer%180==0 then lstg.var.power=min(lstg.var.power+1,400) end
	end
	end
end
function reimu_player:render()
	if lstg.var.power>=200 then
		SetImageState('reimu_support','mul+add',Color(0x80FFFFFF))
		for i=1,4 do
		if self.sp[i] and self.sp[i][3]>0.5 then
			Render('reimu_support',self.supportx+self.sp[i][1],self.supporty+self.sp[i][2],-self.timer*3,1.5)
		end
		end
	end
...
-------------------------------------------------------
reimu_drawring=Class(object)
function reimu_drawring:init()
	self.x=player.x self.y=player.y self.group=GROUP_GHOST self.layer=LAYER_PLAYER-1
	self.omiga=8 self.r=120
end
function reimu_drawring:frame()
	self.x=player.x self.y=player.y
	if self.timer<180 then self.r=self.r-2/3 else Del(self) end
end
function reimu_drawring:render()
	misc.RenderRing('reimu_drawring',self.x,self.y,self.r-6,self.r+6, self.ani*3,32,16)
end
-------------------------------------------------------
