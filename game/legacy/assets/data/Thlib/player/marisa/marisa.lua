...
function marisa_player:init()
	...
	SetImageState('marisa_missile','',Color(0xD0FFFFFF))
	LoadImage('marisa_missile2','marisa_player',192,240,32,16,8,8)
	SetImageState('marisa_missile2','',Color(0xD0FFFFFF))
	...
	LoadImage('marisa_support','marisa_player',128,144,16,16)
	...
	LoadImageGroup('marisa_drawring','boss',64,0,16,8,1,16)
	for i=1,16 do SetImageState('marisa_drawring'..i,'mul+add',Color(0x80FFFFFF)) end
	player_class.init(self)
	self.collect_line=60
	self.name='Marisa'
	self.imgs={}
	self.A=1 self.B=1
...
function marisa_player:shoot()
	...
	if self.slow==1 then
		...
		if self.support>0 then
			local dmg1=3.5
			if IsValid(_boss) then dmg1=1.5 end
			if self.timer%32==0 and self.sp[4] and self.sp[4][3]>0.5 then
				New(marisa_missile_trail,'marisa_missile2',self.supportx+self.sp[1][1],self.supporty+self.sp[1][2],7,3.5,220,self.target,900,dmg1)
				New(marisa_missile_trail,'marisa_missile2',self.supportx+self.sp[4][1],self.supporty+self.sp[4][2],7,3.5,-40,self.target,900,dmg1)
				if not ext.replay.IsReplay() then
					scoredata.marisa_missle=scoredata.marisa_missle+2
				end
				if scoredata.achi_common[19]~=1 and scoredata.marisa_missle>=10000 then
					New(achievement_obj,2,19)
				end
			end
			if self.timer%32==16 and self.sp[4] and self.sp[4][3]>0.5 then
				New(marisa_missile_trail,'marisa_missile2',self.supportx+self.sp[2][1],self.supporty+self.sp[1][2],7,3.5,205,self.target,900,dmg1)
				New(marisa_missile_trail,'marisa_missile2',self.supportx+self.sp[3][1],self.supporty+self.sp[4][2],7,3.5,-25,self.target,900,dmg1)
				if not ext.replay.IsReplay() then
					scoredata.marisa_missle=scoredata.marisa_missle+2
				end
				if scoredata.achi_common[19]~=1 and scoredata.marisa_missle>=10000 then
...
--				local angle=105-i*num
				local angle=self.anglelist[4][i]
				if self.sp[i] and self.sp[i][3]>0.5 then
					...
					local dmg2=0.15
					if IsValid(_boss) then dmg2=0.05 end
					if target then
						self.offset[i]=max(0,self.offset[i]-target.b)
						New(marisa_laser_hit,x+self.offset[i]*cos(angle),y+self.offset[i]*sin(angle))
						if target.class.base.take_damage then target.class.base.take_damage(target,dmg2) end
		...
		New(tasker,function()
			...
			for i=1,120 do
				...
				for i=1,3 do
					...
					for j=1,8 do
						New(player_bullet_hide,16,16,self.x,self.y,12,a+2.1*j,1.2,2*j)
						New(player_bullet_hide,16,16,self.x,self.y,12,a-2.1*j,1.2,2*j)
					end
				end
				task.Wait(2)
...
--		self.lspeed=1
		New(tasker,function()
			for i=1,27 do
				New(marisa_spark_wave,player.x,player.y-16,90,12,1.8)
				task.Wait(10)
			end
			New(bullet_killer,self.x,self.y)
			PlaySound('slash',1.0)
...
---------------------------------
function marisa_player:special()
	if lstg.var.power>=100 then
		scoredata.marisa_c_time=scoredata.marisa_c_time+1
		if scoredata.marisa_c_time==100 then New(achievement_obj,2,20) end
		self.nextsp=180
		lstg.var.power=max(0,lstg.var.power-100)
		PlaySound('hyz_chargeup',0.8)
		New(marisa_drawring)
		self.collect_line=self.collect_line-500
		New(tasker,function()
			task.Wait(89)
			if lstg.var.collectitem[self.item]>=300 then New(achievement_obj,2,21) end
			task.Wait(31)
			self.collect_line=self.collect_line+500
		end)
	end
...
function marisa_player:render()
	local sz=1.35+0.1*sin(self.timer*6)
	local scale=self.ani%90
	--support
	...
	for i=1,4 do if self.sp[i] then
		Render('marisa_support',self.supportx+self.sp[i][1],self.supporty+self.sp[i][2],0,sz)
	end end
	end
...
---------------
marisa_drawring=Class(object)
function marisa_drawring:init()
	self.x=player.x self.y=player.y self.group=GROUP_GHOST self.layer=LAYER_PLAYER-1
	self.omiga=8 self.r=0
end
function marisa_drawring:frame()
	if self.timer<120 then self.r=self.r+9 else Del(self) end
end
function marisa_drawring:render()
	misc.RenderRing('marisa_drawring',self.x,self.y,self.r-6,self.r+6, self.ani*3,32,16)
end
---------------
...
marisa_missile_trail=Class(player_bullet_trail)
function marisa_missile_trail:init(img,x,y,v,_v,angle,target,trail,dmg)
...
function marisa_missile_trail:frame()
	...
	if self.timer>30 then
		self.v=min(self.v+0.5,self.vmax)
		...
		self.vx=self._v*cos(self.angle)
		self.vy=self._v*sin(self.angle)
		self._v=self._v*(1-self.timer/30)
	end
end
function marisa_missile_trail:kill()
	PlaySound('msl2',0.3)
	local a,r=ran:Float(0,360),ran:Float(0,6)
	New(marisa_missile_ef,self.x+r*cos(a),self.y+r*sin(a),self.dmg,5)
end
