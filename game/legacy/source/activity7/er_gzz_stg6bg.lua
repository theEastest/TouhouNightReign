er_gzz_stg6bg_background=Class(object)

function er_gzz_stg6bg_background:init()
...
function er_gzz_stg6bg_background:frame()
	task.Do(self)
	if self.timer>3478+240 then self.layer=LAYER_BG-0.2 end
...
function er_gzz_stg6bg_background:render()
	Set3D('at',0.00,0.00,-0.20)--
	Set3D('up',0.00,1.00,0.00)
	Set3D('fovy',0.62)
	Set3D('z',1.00,4.80)
	...
	if self.timer>3778 then
		Set3D('fog',2.4,2.9,Color(0,0,0,0))
		Set3D('eye',0,0.45,-1.4)
	else
	end
	if self.timer>3478+120 and self.timer<3478+180 then
		self.speed=0.01-0.008*(self.timer-3478-120)/60
	end
	SetViewMode'3d'
