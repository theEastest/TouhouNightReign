starlight2_background=Class(object)

function starlight2_background:init()
	--
...
function starlight2_background:frame()
	...
	if self.acc>=self.interval then
		...
		for _=1,3 do
			...
			y=2+R*sin(a)
			self.listend=self.listend+1
			self.list[self.listend]={rnd(2,3), x,y,rnd()*0.6-0.3,-0.7-0.3*rnd(),rnd(100,110)}
		end
...
function starlight2_background:render()
	Set3D('eye',-1.9,-3.3,-8.6)
	Set3D('at',-0.4,0.9,2.5)
	Set3D('up',1.24,1.1,0.1)
	Set3D('z',2.1,100)
